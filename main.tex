...
Ray tracing is a computer graphics technique that creates images by tracing the path of light from a pixel as it intersects with the objects in the scene, the ray tracer calculates how much light is reflected back along the ray to determine the color of the pixel. Figure 1 illustrates the working principle of the ray tracer we are going to implement. \\
\begin{figure}[ht]
  \centering
...
In order to quantitatively demonstrate the convergence rate between using direct and importance sampling techniques, we collected a sequence of images rendered under different number of samples for both cases. Since each image is comprised of a $768\times 1024$ matrix where every pixel stores a RGB value, by calculating the Mean Square Root Error $\alpha$ = $|$ Tn - I $|$ of every matrix in the sequence of the rendered images, where Tn is the converged matrix and I is the matrix generated at a given number of samples. we can plot the Mean Square Root Error versus the number of samples. The ray tracer program was modified to capture a new image after every sample, however, there are currently some bugs in the code implementation and the generated images are under saturated. The under-saturated image is also atteached below (Figure 4). \\
\begin{figure}[ht]
  \centering
